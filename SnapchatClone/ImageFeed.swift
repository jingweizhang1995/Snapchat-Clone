            ...
            if let posts = snapshot.value as? [String:AnyObject] {
                user.getReadPostIDs(completion: { (ids) in
                    for postKey in posts.keys {
                        ...
                        let postObj = posts[postKey]
                        var objectUser = postObj!.value(forKey: firUsernameNode) as! String
                        var objectImage = postObj!.value(forKey: firImagePathNode) as! String
                        var objectThread = postObj!.value(forKey: firThreadNode) as! String
                        var objectDate = postObj!.value(forKey: firDateNode) as! String
                        let if_read = ids.contains(postKey)
                        
                        var newPost = Post(id: postKey, username: objectUser, postImagePath: objectImage, thread: objectThread, dateString: objectDate, read: if_read)
